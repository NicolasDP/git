...
pub trait Repo {
    ...
    fn get_commit<H: Hash>(&self, h: CommitRef<H>) -> Result<Commit<H>> {
        self.get_object(h)
    }
    fn get_tree<H: Hash>(&self, h: TreeRef<H>) -> Result<Tree<H>> {
        self.get_object(h)
    }
    fn get_blob<H: Hash>(&self, h: BlobRef<H>) -> Result<Blob> {
        self.get_object(h)
    }
